...
const ROLES = ['viewer', 'prompter', 'host'];
...
test('Enter and editing keys route to the draft editor for prompter+', () => {
  for (const role of ['prompter', 'host']) {
...
test('y/n answers an ask (→ pty) only for prompter+ and only while armed', () => {
  // armed: prompter+ answer (the driver tier is folded in), viewer blocked
  assert.deepEqual(d('u', 'prompter', 'y', { armed: true }), { kind: 'pty', data: 'y' });
  assert.deepEqual(d('u', 'host', 'n', { armed: true }), { kind: 'pty', data: 'n' });
  assert.deepEqual(d('u', 'host', 'Y', { armed: true }), { kind: 'pty', data: 'Y' });
  assert.equal(d('u', 'prompter', 'y', { armed: true, composing: true }).kind, 'draft', 'composing y is just typing');
  ...
  assert.equal(d('u', 'prompter', 'y', { armed: false }).kind, 'draft');
...
test('mode flip (Shift+Tab) is prompter+ (drivers are gone), dropped for viewer', () => {
  assert.deepEqual(d('u', 'prompter', SHIFT_TAB), { kind: 'pty', data: SHIFT_TAB });
...
test('classifySend distinguishes command / claude-slash / bash / prompt', () => {
  assert.deepEqual(classifySend('/role @a prompter'), { kind: 'command', name: 'role' });
...
test('sendAllowed gates prompt, claude-slash & bash (all prompter+)', () => {
  ...
  for (const kind of ['claude-slash', 'bash']) {
    assert.equal(sendAllowed(kind, 'viewer'), false, `${kind} not for viewer`);
    assert.equal(sendAllowed(kind, 'prompter'), true, kind);
